...
export default function CarrosAgentePage() {
  ...
  useEffect(() => {
    const fetchUserCars = async () => {
      ...
      if (!error && data) {
        ...
        setCars(normalized);
        setLocations([...new Set(normalized.map((car) => car.location))]);
      } else {
  ...
  const handleSolicitudBaja = async (id) => {
    ...
    try {
      ...
      setCars((prev) =>
        prev.map((c) =>
          c.id === id ? { ...c, estado: "Solicitud de baja" } : c
        )
      );
    } catch (error) {
  ...
  return (
    <div className="px-4 py-8 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-6">
        <h1 className="text-2xl font-bold text-[#0B1D3B]">Mis Carros Publicados</h1>
        <Button
          className="bg-[#0B1D3B] text-white hover:bg-[#13294b] flex items-center gap-2"
          onClick={() => router.push("/agente/publicar-carro")}
        >
        ...
        {paginatedCars.map((car) => (
          ...
          >
            ...
            <CardContent>
              ...
              <h3 className="text-md font-semibold text-[#0B1D3B]">{car.title}</h3>
              <p className="text-sm text-gray-500">{car.location}</p>
              <p className="text-sm text-gray-500">
                {car.marca} {car.modelo} | {car.puertas} puertas |{" "}
                {car.transmision} | {car.combustible}
              </p>
              ...
              <div className="flex gap-4 mt-2">
                <Button
                  onClick={() => handleSolicitudBaja(car.id)}
